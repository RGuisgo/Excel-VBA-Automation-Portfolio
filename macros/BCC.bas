...
Sub BCCMacro()
    ...
    ' Fill remaining empty cells in column AF with "ic"
    With Workbooks(WorkbookSelected).Worksheets(1)
        For j = 2 To lastRow1
            If IsEmpty(.Range("AF" & j).Value) Then
                .Range("AF" & j).Value = "Interco"
                'Debug.Print "Cell AF" & j & " was empty, set to 'ic'"
            End If
        Next j
    End With
